...
trainer (`../mnist.c`) produces `mnist_model.h` (weights) and `mnist_samples.bin`
(held-out samples); a target consumes those.
...
2. Compile against `../../../../client/nn_infer.c` and the generated
   `../../mnist_model.h` (define `NN_MODEL_HEADER`, or copy it in as
   `nn_model.h`).
3. Feed inputs scaled **identically to training** (MNIST divides each pixel by
   `512.0`).
