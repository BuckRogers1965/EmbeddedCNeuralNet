// Embedded inference client for MNIST, run on the desktop as an end-to-end test
// of the WHOLE deploy path. Links ONLY against the pure-C client in client/
// (no BLAS, no SIMD, no training code) plus the generated mnist_model.h --
// exactly what you would compile into firmware. It replays the samples the
// trainer dumped and confirms nn_classify() reproduces the training library's
// predictions, proving train -> export -> deploy is faithful.
//
...
int main(int argc, char **argv) {
    const char *path = (argc > 1) ? argv[1] : "mnist_samples.bin";
    FILE *f = fopen(path, "rb");
    if (!f) { perror(path); return 1; }

    int n = 0, isz = 0;
    if (fread(&n, sizeof(int), 1, f) != 1 || fread(&isz, sizeof(int), 1, f) != 1) {
        fprintf(stderr, "mnist_samples.bin: bad header\n"); return 1;
    }
    ...
    for (int s = 0; s < n; ++s) {
        ...
        if (fread(x, sizeof(float), isz, f) != (size_t)isz ||
            ...
            fprintf(stderr, "mnist_samples.bin: truncated at sample %d\n", s); return 1;
        }
        int got = nn_classify(x);
        if (shown < 5) { printf("  client sample %d -> digit %d\n", s, got); ++shown; }
