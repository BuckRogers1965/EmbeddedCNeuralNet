...
typedef double (*ErrorFunction)(double output, double expected);
typedef double (*ErrorFunctionDerivative)(NeuralNet *net, double output, double expected);
...
double nothing(NeuralNet *net, double output, double expected)
{
    return 1;
}

double mse_error(NeuralNet *net, double output, double expected)
{
    double diff = output - expected;
    return 0.5 * diff * diff ;
}

double mse_error_derivative(NeuralNet *net, double output, double expected)
{
    return output - expected;
}

double cross_entropy_error(NeuralNet *net, double output, double expected)
{
    return (-expected * log(output + 1e-15) - (1 - expected) * log(1 - output + 1e-15 ));
}

double cross_entropy_error_derivative(NeuralNet *net, double output, double expected)
{
    ...
    for (i = net->num_layers - 1; i > 0; --i) {
        ...
        for (j = 0; j < current_layer->output_size; ++j) {
            for (k = 0; k < current_layer->input_size; k += 2) {
                ...
                // Use SSE2 to update weights in pairs
                __m128d grad = _mm_set_pd(gradient1, gradient0);
                __m128d weight = _mm_loadu_pd(&current_layer->weights[weight_index]);
                __m128d update = _mm_sub_pd(weight, _mm_mul_pd(grad, _mm_set1_pd(net->learningrate)));
            ...
            // Update biases
            double bias_update = current_layer->errors[j] / net->batchsize;
            current_layer->biases[j] -= net->learningrate * bias_update;
        }
...
void backward_pass_revised(NeuralNet *net, double *input, double *expected, double *output) {
    int i, j, k;
    Layer *output_layer = &net->layers[net->num_layers - 1];
    ...
    for (i = net->num_layers - 1; i > 0; --i) {
        ...
        for (j = 0; j < current_layer->output_size; ++j) {
            ...
            net->chooser(net->opt_params, current_layer->biases, i, j, current_layer->errors[j] / net->batchsize - jitter(net), 
                    net->learningrate);
...
void backward_passbad(NeuralNet *net, double *input, double *expected, double *output)
{
    int i, j, k;
    Layer *output_layer = &net->layers[net->num_layers - 1];
    ...
    else
    {
        ...
        {   
            output_layer->errors[i] = (output[i] - expected[i]) * output_layer->activate_derivative(output[i]);
            //output_layer->errors[i] = net->calculate_error_derivative(output[i], expected[i]) *
                                      //output_layer->activate_derivative(output[i]);
        }
    }
        ...
        if (i > 1)
        {
            for (j = 0; j < prev_layer->output_size; ++j)
            {
                ...
                for (k = 0; k < current_layer->output_size; ++k)
                {
                    //prev_layer->errors[j] += current_layer->errors[k] *
                                             //current_layer->weights[k * current_layer->input_size + j];
                    prev_layer->errors[j] += net->calculate_error_derivative(net, output[j], expected[j]) * current_layer->errors[k] *
                                             current_layer->weights[k * current_layer->input_size + j];
                }
                // prev_layer->errors[j] *= prev_layer->activate_derivative(prev_layer->activations[j]);
                //prev_layer->errors[j] = net->calculate_error_derivative(output[j], expected[j]) *
                                          //prev_layer->activate_derivative(prev_layer->activations[j]);
                prev_layer->errors[j] *= net->calculate_error_derivative(net, output[j], expected[j]) * prev_layer->activate_derivative(prev_layer->activations[j]);
            }
        }
    }
}
...
void train(NeuralNet *net, double **images, double **labels, int trainsize)
{
    ...
    if (net->opt_params == NULL)
        setup_chooser_params(net);
